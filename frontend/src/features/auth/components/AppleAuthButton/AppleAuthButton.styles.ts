 ...
 * `style`, so the height comes from the shared pill scale - the same one the
 * Google button beside it is drawn at - and the radius is handed over as the
 * `cornerRadius` prop instead.
 */
export const createAppleAuthButtonStyles = (theme: Theme): AppleAuthButtonStyleMap =>
  StyleSheet.create({
    button: { height: theme.size.pillMedium, alignSelf: 'stretch' },
  });
 ...
 */
export const appleCornerRadius = (theme: Theme): number => theme.size.pillMedium / CORNER_HALVES;
