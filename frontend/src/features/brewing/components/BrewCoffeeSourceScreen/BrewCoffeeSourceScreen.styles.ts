...
type BrewCoffeeSourceStyleMap = ViewStyles<'content' | 'choices' | 'grid' | 'options' | 'actions'>;
...
export const createBrewCoffeeSourceStyles = (theme: Theme): BrewCoffeeSourceStyleMap =>
  StyleSheet.create({
    ...
    /** Two rows of tiles, the same distance apart as the tiles in a row. */
    grid: { gap: theme.layout.cardGap },
